...
bool areTreesSimilarInShape(Node* tree1, Node* tree2) {
    if (tree1->children.size() != tree2->children.size()) {
        return false;
    }
    for (int i = 0; i < tree1->children.size(); i++) {
        bool childans = areTreesSimilarInShape(tree1->children[i], tree2->children[i]);
...
int main() {
    ...
    cout << areTreesSimilarInShape(tree1, tree2);
}